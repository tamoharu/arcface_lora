...
def load_params(layer, weight, bias):
    ...
    layer.weight.data = torch.from_numpy(numpy_helper.to_array(weight).copy())
    if bias is not None:
        layer.bias.data = torch.from_numpy(numpy_helper.to_array(bias).copy())
...
def convert_batch_norm_layer(node, params):
    ...
    torch_params = [torch.from_numpy(numpy_helper.to_array(param).copy()) for param in params]
    ...
    for par_ix, par_name in enumerate(node.input):
        if par_ix == 0:
            if par_name in weights:
                X = torch.from_numpy(numpy_helper.to_array(weights[par_name]).copy())
        elif par_ix == 1:
            if par_name in weights:
                W = torch.from_numpy(numpy_helper.to_array(weights[par_name]).copy())
        elif par_ix == 2:
            if par_name in weights:
                R = torch.from_numpy(numpy_helper.to_array(weights[par_name]).copy())
        elif par_ix == 3:
            if par_name != "" and par_name in weights:
                B = torch.from_numpy(numpy_helper.to_array(weights[par_name]).copy())
        elif par_ix == 4:
            if par_name != "" and par_name in weights:
                sequence_lens = torch.from_numpy(
                    numpy_helper.to_array(weights[par_name].copy())
                )
        elif par_ix == 5:
            if par_name != "" and par_name in weights:
                initial_h = torch.from_numpy(numpy_helper.to_array(weights[par_name]).copy())
        elif par_ix == 6:
            if par_name != "" and par_name in weights:
                initial_c = torch.from_numpy(numpy_helper.to_array(weights[par_name]).copy())
        elif par_ix == 7:
            if par_name != "" and par_name in weights:
                P = torch.from_numpy(numpy_helper.to_array(weights[par_name]).copy())
